...
export class McpError extends AppError {
    ...
    isFallbackTriggered;
    originalError;
    constructor(message, code, service, options) {
...
/** 根据原始错误创建MCP错误 */
export function createMcpError(error, service, context) {
    // 判断错误类型
    let code;
    let message;
...
export async function executeWithFallback(operation, fallback, options) {
    ...
    try {
        ...
        const mcpError = error instanceof McpError ? error : createMcpError(error, 'unknown');
...
/** 创建降级错误（用于记录降级事件） */
export function createFallbackError(service, originalError, fallbackReason) {
    return new McpError(`MCP服务降级: ${fallbackReason}`, McpErrorCode.FALLBACK_TRIGGERED, service, {
        originalError,
...
// ==================== 预定义错误 ====================
/** 时间服务不可用错误 */
export function timeServiceUnavailableError(error) {
    return new McpError('时间服务暂时不可用', McpErrorCode.SERVICE_UNAVAILABLE, 'time', { originalError: error });
}
/** 记忆服务不可用错误 */
export function memoryServiceUnavailableError(error) {
    return new McpError('记忆服务暂时不可用', McpErrorCode.SERVICE_UNAVAILABLE, 'memory', { originalError: error });
}
/** API密钥无效错误 */
export function invalidApiKeyError(service) {
    return new McpError('API密钥无效或已过期', McpErrorCode.AUTHENTICATION_FAILED, service);
}
/** 请求超时错误 */
export function requestTimeoutError(service, timeoutMs) {
    return new McpError(`请求超时 (${timeoutMs}ms)`, McpErrorCode.TIMEOUT, service);
}
