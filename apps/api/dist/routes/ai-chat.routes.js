...
const cleanJsonText = (s) => s.replace(/^\uFEFF/, '').trim();
...
router.post('/ask', (req, _res, next) => {
    ...
    try {
            ...
            execFile(PYTHON_EXE, [pythonScript, String(question)], (error, stdout, stderr) => {
                ...
                try {
                    ...
                    const chunksText = rawChunks
                        ...
                        .filter(Boolean);
                    // ✅ 过滤明显是目录/导航
                    const isJunk = (t) => /目录|上一篇|下一篇|连载|排版|撰文|责任编辑|点击阅读|更多内容/.test(t);
                    ...
                    process.stdout.write(`🧠 chunk0=${(filteredChunks[0] || chunksText[0] || '').slice(0, 120)}\n`);
                    // ✅ 真正喂给 DeepSeek 的上下文
         ...
         */
        const contextText = kb.ragContext && kb.ragContext.trim().length > 0
            ? kb.ragContext
            : '（检索未命中任何相关片段）';
        // ============ MCP集成：获取记忆和时间 ============
...
- 非医疗诊断`;
        ...
        // 构建引用链接信息
        const references = kb.rawChunks.map((chunk, index) => {
