...
export class TimeService {
     ...
     * @returns 当前时间信息
     */
    async getCurrentTime(userId, forceRefresh = false) {
        // 构建缓存键
     ...
     * @returns 格式化后的时间字符串
     */
    async getFormattedCurrentTime(format, userId) {
    ...
    /** 格式化时间响应 */
    formatTime(timeResponse, format) {
...
/** 重置时间服务实例（主要用于测试） */
export function resetTimeService() {
 ...
 * 获取当前时间（快捷函数）
 */
export async function getCurrentTime(userId) {
    return getTimeService().getCurrentTime(userId);
}
/**
 * 获取当前时间戳（快捷函数）
 */
export async function getCurrentTimestamp(userId) {
    return getTimeService().getTimestamp(userId);
}
/**
 * 获取当前格式化时间（快捷函数）
 */
export async function getCurrentFormattedTime(format, userId) {
 ...
 * 检查时间是否在最近N分钟内
 */
export function isWithinMinutes(time, minutes, referenceTime) {
 ...
 * 将时间响应转换为Date对象
 */
export function toDate(timeResponse) {
    return new Date(timeResponse.current_time);
}
/**
 * 将Date对象转换为时间响应
 */
export function fromDate(date, timezone = 'Asia/Shanghai') {
    return {
