...
export class MedicalHistoryService {
     ...
     * @returns 病史报告数据
     */
    async generateMedicalHistory(userId, options = {}) {
        const { format = 'pdf', timeRange, includeMemories = true, includeTimeline = true, patientInfo = {}, } = options;
        // 确定时间范围
        ...
        const memories = includeMemories
            ? await this.memoryManager.retrieveMemories({
                ...
                endDate,
                limit: 100, // 最多100条记忆
                types: ['symptom', 'conversation', 'medical_event'],
            })
            : { memories: [], total: 0 };
        // 构建症状时间线
        ...
        const memorySummary = includeMemories
            ...
            : [];
        // 生成总结和建议
        const summary = await this.generateReportSummary(symptomTimeline, memorySummary, patientInfo);
        const recommendations = await this.generateRecommendations(symptomTimeline, memorySummary);
        // 获取当前时间
     ...
     * @returns 导出文件路径和元数据
     */
    async exportMedicalHistory(userId, report, format) {
        // 确保导出目录存在
     ...
     * @returns 导出记录ID
     */
    async saveExportRecord(userId, exportData) {
        const pool = getPool();
        const { format, filePath, fileSize, downloadUrl, memoryIds = [], exportOptions = {}, expiresAt, } = exportData;
        // 确定导出类型（基于选项）
        ...
        const result = await pool.query(`INSERT INTO medical_history_exports (
     ...
     * @returns 导出记录列表
     */
    async getExportHistory(userId, limit = 20, offset = 0) {
        const pool = getPool();
        const result = await pool.query(`SELECT * FROM medical_history_exports 
       WHERE user_id = $1 
    ...
    async incrementDownloadCount(exportId) {
        const pool = getPool();
        await pool.query(`UPDATE medical_history_exports 
    ...
    async cleanupExpiredExports() {
        const pool = getPool();
        const result = await pool.query(`DELETE FROM medical_history_exports 
    ...
    // ==================== 内部辅助方法 ====================
    /** 构建症状时间线 */
    async buildSymptomTimeline(userId, startDate, endDate) {
        // 获取症状时间线条目
        const pool = getPool();
        const result = await pool.query(`SELECT * FROM symptom_timeline 
       WHERE user_id = $1 
         AND occurred_at BETWEEN $2 AND $3
       ORDER BY occurred_at DESC`, [userId, startDate, endDate]);
        // 按症状分组
    ...
    /** 构建记忆摘要 */
    async buildMemorySummary(memories) {
        return memories.slice(0, 20).map((memory) => ({
            ...
            symptoms: memory.content?.extractedSymptoms?.map((s) => s.description) || [],
            keyPoints: [
    ...
    /** 生成报告总结 */
    async generateReportSummary(symptomTimeline, memorySummary, patientInfo) {
    ...
    /** 生成建议 */
    async generateRecommendations(symptomTimeline, memorySummary) {
    ...
    /** 生成症状描述 */
    generateSymptomDescription(description, entries) {
    ...
    /** 导出为JSON */
    async exportToJson(report, filePath) {
        const jsonContent = JSON.stringify(report, null, 2);
        await fs.writeFile(filePath, jsonContent);
    }
    /** 导出为文本 */
    async exportToText(report, filePath) {
    ...
    generateTextContent(report) {
        ...
        if (report.patientInfo.name)
            lines.push(`姓名: ${report.patientInfo.name}`);
        if (report.patientInfo.age)
            lines.push(`年龄: ${report.patientInfo.age}`);
        if (report.patientInfo.gender)
            lines.push(`性别: ${report.patientInfo.gender}`);
...
// ==================== 导出单例实例 ====================
/** 全局病史导出服务实例 */
let medicalHistoryServiceInstance = null;
/** 获取病史导出服务实例（单例模式） */
export function getMedicalHistoryService() {
...
/** 重置病史导出服务实例（主要用于测试） */
export function resetMedicalHistoryService() {
