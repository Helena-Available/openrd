...
import type { McpConfig, McpServiceConfig, TimeServiceConfig } from '../types/mcp.types.js';
...
/** 默认MCP服务配置 */
const DEFAULT_SERVICE_CONFIG: McpServiceConfig = {
  endpoint: '',
  timeout: 10000, // 10秒
  retries: 3,
  retryDelay: 1000, // 1秒
...
/** 默认时间服务配置 */
const DEFAULT_TIME_SERVICE_CONFIG: TimeServiceConfig = {
...
/** 默认记忆服务配置 */
const DEFAULT_MEMORY_SERVICE_CONFIG: McpServiceConfig = {
...
export function loadMcpConfig(): McpConfig {
  ...
  // 时间服务配置
  const timeServiceConfig: McpServiceConfig & TimeServiceConfig = {
  ...
  // 记忆服务配置
  const memoryServiceConfig: McpServiceConfig = {
...
/** 全局MCP配置实例 */
let mcpConfigInstance: McpConfig | null = null;
